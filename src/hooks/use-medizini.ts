...
export function useMedizini() {
  ...
  // Increments the active medizini's dose progress and advances stage when threshold is reached.
  // Returns whether a stage advancement happened and what the new stage is.
  const confirmDoseProgress = useCallback(async (): Promise<{
    advanced: boolean;
    newStage: MediziniStage | null;
  }> => {
    if (!medizini) return { advanced: false, newStage: null };

    const newProgress = medizini.current_doses_progress + 1;
